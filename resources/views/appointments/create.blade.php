<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:12px">
            <div>
                <h4 class="font-weight-bold mb-0"><i class="mdi mdi-calendar-plus text-primary mr-1"></i>Book Appointment</h4>
                <small class="text-muted">Schedule a new patient consultation</small>
            </div>
            <a href="{{ route('appointments.index') }}" class="btn btn-light btn-sm"><i class="mdi mdi-arrow-left"></i> Back to List</a>
        </div>
    </x-slot>
                ...
                <div class="col-lg-8">
                    ...
                    <div class="data-card mb-3">
                        ...
                        <select name="patient_id" required class="form-control" x-model="patientId" @change="onPatientChange()">
                            ...
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}" {{ old('patient_id', $selectedPatient) == $patient->id ? 'selected' : '' }}>{{ $patient->patient_id }} — {{ $patient->name }}@if($patient->phone) · {{ $patient->phone }}@endif</option>
                            @endforeach
                        </select>
                    ...
                    <div class="data-card mb-3">
                        <div class="d-flex align-items-center mb-3" style="gap:10px">
                            ...
                            <div>
                                ...
                                <small class="text-muted">Choose a doctor — schedule shown live</small>
                            </div>
                        </div>
                        <select name="doctor_id" required class="form-control" x-model="doctorId" @change="onDoctorChange()">
                            <option value="">— Select doctor —</option>
                            @foreach($doctors as $doc)
                                <option value="{{ $doc->id }}" {{ old('doctor_id', $selectedDoctor) == $doc->id ? 'selected' : '' }}>Dr. {{ $doc->user->name }} — {{ $doc->specialization ?? 'GP' }} (RM {{ number_format($doc->consultation_fee ?? 0, 2) }})</option>
                            @endforeach
                        </select>
                        @error('doctor_id')<small class="text-danger">{{ $message }}</small>@enderror

                        {{-- Weekly schedule chips --}}
                        <div x-show="doctor.schedule && Object.keys(doctor.schedule).length" class="mt-3">
                    ...
                    <div class="data-card mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-2">
                                ...
                                <input type="date" name="appointment_date" required class="form-control" x-model="apptDate" :min="today" />
                    ...
                    <div class="data-card mb-3">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" name="reason" class="form-control" value="{{ old('reason') }}" placeholder="e.g. Fever, follow-up, check-up" />
                        </div>
                        <div>
                            <label class="form-label small font-weight-bold">Notes (internal)</label>
                            <textarea name="notes" rows="2" class="form-control" placeholder="Any internal notes for staff">{{ old('notes') }}</textarea>
                        </div>
                    </div>

                    <div class="d-flex" style="gap:8px">
                        <button type="submit" class="btn btn-primary font-weight-bold"><i class="mdi mdi-check-circle"></i> Book Appointment</button>
    ...
    <script>
        ...
        function apptForm() {
            return {
                ...
                apptDate: '{{ old('appointment_date', date('Y-m-d')) }}',
                startTime: '{{ old('start_time', '09:00') }}',
                endTime: '{{ old('end_time', '09:30') }}',
                today: '{{ date('Y-m-d') }}',
                patient: {},
                ...
                onPatientChange() {
                    this.patient = PATIENT_MAP[this.patientId] || {};
                },
                onDoctorChange() {
                    this.doctor = DOCTOR_MAP[this.doctorId] || { fee: 0, schedule: {} };
                },
                autoEndTime() {
                ...
                get dayLabel() {
                    return this.dayName ? this.dayName : '';
                },
                get dateMonth() {
                    if (!this.apptDate) return '';
