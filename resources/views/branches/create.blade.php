<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:12px">
            <div>
                <h4 class="font-weight-bold mb-0"><i class="mdi mdi-hospital-building text-primary mr-1"></i>Add New Branch</h4>
                <small class="text-muted">Open a new clinic location</small>
            </div>
            <a href="{{ route('branches.index') }}" class="btn btn-light btn-sm"><i class="mdi mdi-arrow-left"></i> Back to Branches</a>
        </div>
    </x-slot>
                ...
                <div class="col-lg-8">
                    ...
                    <div class="data-card mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-7 mb-2">
                                ...
                                <input type="text" name="name" required class="form-control" x-model="name" @input="autoCode()" placeholder="ClinicQo Subang Jaya" value="{{ old('name') }}" />
                                @error('name')<small class="text-danger">{{ $message }}</small>@enderror
                            </div>
                            <div class="col-md-5 mb-2">
                                <label class="form-label small font-weight-bold">Branch Code *</label>
                                <input type="text" name="code" required class="form-control text-uppercase" x-model="code" maxlength="10" placeholder="KU-SA" value="{{ old('code') }}" style="letter-spacing:0.1em;font-weight:600" />
                    ...
                    <div class="data-card mb-3">
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="address" rows="2" class="form-control" x-model="address" placeholder="Lot 12, Jalan SS15/4, Subang Jaya, 47500 Selangor">{{ old('address') }}</textarea>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="phone" class="form-control" x-model="phone" placeholder="+60 3-1234 5678" value="{{ old('phone') }}" />
                                </div>
                            </div>
                            <div class="col-md-6 mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="email" name="email" class="form-control" x-model="email" placeholder="branch@clinicqo.my" value="{{ old('email') }}" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="data-card mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="time" name="opening_time" class="form-control" x-model="opening" value="{{ old('opening_time', '09:00') }}" />
                            </div>
                            <div class="col-md-6 mb-2">
                                <label class="form-label small font-weight-bold">Closing *</label>
                                <input type="time" name="closing_time" class="form-control" x-model="closing" value="{{ old('closing_time', '18:00') }}" />
                            </div>
                        </div>
                    ...
                    <div class="data-card mb-3">
                        ...
                        <label class="d-flex align-items-center mb-0 p-3" style="gap:12px;cursor:pointer;background:#f8fafc;border-radius:8px;border:1px solid #e5e7eb">
                            <input type="checkbox" name="is_active" value="1" x-model="active" {{ old('is_active', true) ? 'checked' : '' }} style="display:none">
                    ...
                    <div class="d-flex" style="gap:8px">
                        <button type="submit" class="btn btn-primary font-weight-bold"><i class="mdi mdi-check-circle"></i> Create Branch</button>
                ...
                <div class="col-lg-4">
                    <div class="data-card" style="position:sticky;top:80px">
                        ...
                        <div class="mt-3 p-3" style="background:#fffbeb;border-radius:10px;border:1px solid #fde68a">
                            ...
                            <div class="text-center mt-2" style="font-size:12px;color:#78350f">
    ...
    <script>
        function branchForm() {
            return {
                ...
                phone: '{{ old('phone') }}',
                email: '{{ old('email') }}',
                opening: '{{ old('opening_time', '09:00') }}',
                closing: '{{ old('closing_time', '18:00') }}',
                active: {{ old('is_active', true) ? 'true' : 'false' }},
                init() {},
                autoCode() {
                    if (this.code) return;
                    const parts = this.name.split(/\s+/).filter(Boolean);
                    if (parts.length >= 2) this.code = (parts[parts.length - 2][0] + parts[parts.length - 1][0]).toUpperCase();
                    else if (parts[0]) this.code = parts[0].slice(0, 3).toUpperCase();
                },
