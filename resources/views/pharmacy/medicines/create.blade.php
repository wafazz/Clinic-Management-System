<x-app-layout>
    <x-slot name="header"><h4 class="font-weight-bold mb-0">Add Medicine</h4></x-slot>
            ...
            <form method="POST" action="{{ route('medicines.store') }}" >
                    ...
                    <div>
                        ...
                        <select name="pharmacy_category_id" class="form-control">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('pharmacy_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="unit" required class="form-control">
                            @foreach(['tablet', 'capsule', 'ml', 'bottle', 'tube', 'sachet', 'vial', 'ampoule', 'piece'] as $u)
                                <option value="{{ $u }}" {{ old('unit', 'tablet') === $u ? 'selected' : '' }}>{{ ucfirst($u) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row">
                    <div>
                        ...
                        <input type="number" step="0.01" name="cost_price" value="{{ old('cost_price', '0.00') }}" required class="form-control" />
                    </div>
                    <div>
                        <label class="form-label">Selling Price (RM) *</label>
                        <input type="number" step="0.01" name="selling_price" value="{{ old('selling_price', '0.00') }}" required class="form-control" />
                    </div>
                    <div>
                        <label class="form-label">Current Stock *</label>
                        <input type="number" name="current_stock" value="{{ old('current_stock', 0) }}" required class="form-control" />
                    </div>
                </div>
                <div class="row">
                    <div>
                        <label class="form-label">Reorder Level</label>
                        <input type="number" name="reorder_level" value="{{ old('reorder_level', 10) }}" class="form-control" />
                    </div>
                    <div>
